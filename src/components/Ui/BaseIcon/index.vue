<template>
  <svg
    v-if="svgName === 'setting'"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 24 24"
    aria-hidden="true"
    :fill="fill"
    :width="width"
    :hidden="height"
    :class="className"
  >
    <path
      d="M12,8A4,4 0 0,1 16,12A4,4 0 0,1 12,16A4,4 0 0,1 8,12A4,4 0 0,1 12,8M12,10A2,2 0 0,0 10,12A2,2 0 0,0 12,14A2,2 0 0,0 14,12A2,2 0 0,0 12,10M10,22C9.75,22 9.54,21.82 9.5,21.58L9.13,18.93C8.5,18.68 7.96,18.34 7.44,17.94L4.95,18.95C4.73,19.03 4.46,18.95 4.34,18.73L2.34,15.27C2.21,15.05 2.27,14.78 2.46,14.63L4.57,12.97L4.5,12L4.57,11L2.46,9.37C2.27,9.22 2.21,8.95 2.34,8.73L4.34,5.27C4.46,5.05 4.73,4.96 4.95,5.05L7.44,6.05C7.96,5.66 8.5,5.32 9.13,5.07L9.5,2.42C9.54,2.18 9.75,2 10,2H14C14.25,2 14.46,2.18 14.5,2.42L14.87,5.07C15.5,5.32 16.04,5.66 16.56,6.05L19.05,5.05C19.27,4.96 19.54,5.05 19.66,5.27L21.66,8.73C21.79,8.95 21.73,9.22 21.54,9.37L19.43,11L19.5,12L19.43,13L21.54,14.63C21.73,14.78 21.79,15.05 21.66,15.27L19.66,18.73C19.54,18.95 19.27,19.04 19.05,18.95L16.56,17.95C16.04,18.34 15.5,18.68 14.87,18.93L14.5,21.58C14.46,21.82 14.25,22 14,22H10M11.25,4L10.88,6.61C9.68,6.86 8.62,7.5 7.85,8.39L5.44,7.35L4.69,8.65L6.8,10.2C6.4,11.37 6.4,12.64 6.8,13.8L4.68,15.36L5.43,16.66L7.86,15.62C8.63,16.5 9.68,17.14 10.87,17.38L11.24,20H12.76L13.13,17.39C14.32,17.14 15.37,16.5 16.14,15.62L18.57,16.66L19.32,15.36L17.2,13.81C17.6,12.64 17.6,11.37 17.2,10.2L19.31,8.65L18.56,7.35L16.15,8.39C15.38,7.5 14.32,6.86 13.12,6.62L12.75,4H11.25Z"
    ></path>
  </svg>
  <svg
    v-if="svgName === 'profile'"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 24 24"
    aria-hidden="true"
    :fill="fill"
    :width="width"
    :hidden="height"
    :class="className"
  >
    <path
      d="M12,4A4,4 0 0,1 16,8A4,4 0 0,1 12,12A4,4 0 0,1 8,8A4,4 0 0,1 12,4M12,6A2,2 0 0,0 10,8A2,2 0 0,0 12,10A2,2 0 0,0 14,8A2,2 0 0,0 12,6M12,13C14.67,13 20,14.33 20,17V20H4V17C4,14.33 9.33,13 12,13M12,14.9C9.03,14.9 5.9,16.36 5.9,17V18.1H18.1V17C18.1,16.36 14.97,14.9 12,14.9Z"
    ></path>
  </svg>
  <svg
    v-if="svgName === 'close'"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 16 16"
    :fill="fill"
    :width="width"
    :hidden="height"
    :class="className"
  >
    <path
      d="M12.2 3.80664C12.0754 3.6818 11.9063 3.61165 11.73 3.61165C11.5536 3.61165 11.3845 3.6818 11.26 3.80664L7.99998 7.05997L4.73998 3.79997C4.61543 3.67514 4.44633 3.60498 4.26998 3.60498C4.09364 3.60498 3.92453 3.67514 3.79998 3.79997C3.53998 4.05997 3.53998 4.47997 3.79998 4.73997L7.05998 7.99997L3.79998 11.26C3.53998 11.52 3.53998 11.94 3.79998 12.2C4.05998 12.46 4.47998 12.46 4.73998 12.2L7.99998 8.93997L11.26 12.2C11.52 12.46 11.94 12.46 12.2 12.2C12.46 11.94 12.46 11.52 12.2 11.26L8.93998 7.99997L12.2 4.73997C12.4533 4.48664 12.4533 4.05997 12.2 3.80664Z"
    />
  </svg>
  <svg
    v-if="svgName === 'arrow'"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 24 24"
    :fill="fill"
    :width="width"
    :hidden="height"
    :class="className"
  >
    <path
      d="M15.8751 9.00001L11.9951 12.88L8.1151 9.00001C7.92827 8.81275 7.67462 8.70752 7.4101 8.70752C7.14558 8.70752 6.89193 8.81275 6.7051 9.00001C6.3151 9.39001 6.3151 10.02 6.7051 10.41L11.2951 15C11.6851 15.39 12.3151 15.39 12.7051 15L17.2951 10.41C17.6851 10.02 17.6851 9.39001 17.2951 9.00001C16.9051 8.62001 16.2651 8.61001 15.8751 9.00001Z"
      fill="#9DA9B8"
    />
  </svg>
  <svg
    v-if="svgName === 'main-logo'"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 49 48"
    :fill="fill"
    :width="width"
    :hidden="height"
    :class="className"
  >
    <path
      d="M1.98441 29.2899C1.98441 27.0299 2.42954 24.7919 3.29444 22.704C4.15935 20.6159 5.42701 18.7187 7.02513 17.1206C8.62324 15.5225 10.5204 14.2548 12.6084 13.3899C14.6965 12.5251 16.9344 12.0799 19.1945 12.0799V29.2899H1.98441Z"
      class="ccustom"
      fill="#442781"
    ></path>
    <path
      d="M1.98441 29.2899C1.98441 31.55 2.42954 33.7879 3.29444 35.876C4.15935 37.964 5.42701 39.8612 7.02513 41.4593C8.62324 43.0574 10.5204 44.3251 12.6084 45.19C14.6965 46.0549 16.9344 46.5 19.1945 46.5V29.2899H1.98441Z"
      class="ccompli1"
      fill="#61459C"
    ></path>
    <path
      d="M36.4043 29.2899C36.4043 31.55 35.9595 33.7879 35.0947 35.876C34.2298 37.964 32.9622 39.8612 31.3638 41.4593C29.7657 43.0574 27.8685 44.3251 25.7804 45.19C23.6925 46.0549 21.4545 46.5 19.1945 46.5V29.2899H36.4043Z"
      class="ccompli2"
      fill="#A992DB"
    ></path>
    <path
      d="M47.0156 14.422C47.0156 21.5586 41.23 27.344 34.0935 27.344H21.1716V14.422C21.1716 7.2854 26.957 1.5 34.0935 1.5C41.23 1.5 47.0156 7.2854 47.0156 14.422Z"
      class="ccustom"
      fill="#FF7917"
    ></path>
  </svg>
  <svg
    v-if="svgName === 'check'"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 18 18"
    width="18"
    height="18"
    role="presentation"
    fill="currentColor"
  >
    <path
      d="M16.145,2.571c-0.272-0.273-0.718-0.273-0.99,0L6.92,10.804l-4.241-4.27
      c-0.272-0.274-0.715-0.274-0.989,0L0.204,8.019c-0.272,0.271-0.272,0.717,0,0.99l6.217,6.258c0.272,0.271,0.715,0.271,0.99,0
      L17.63,5.047c0.276-0.273,0.276-0.72,0-0.994L16.145,2.571z"
    />
  </svg>
  <svg
    v-if="svgName === 'refresh'"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 16 16"
    width="18"
    height="18"
    role="presentation"
    :fill="fill"
  >
    <path
      d="M8.00013 3.33335V2.14002C8.00013 1.84002 7.64013 1.69335 7.43346 1.90669L5.5668 3.76669C5.43346 3.90002 5.43346 4.10669 5.5668 4.24002L7.4268 6.10002C7.64013 6.30669 8.00013 6.16002 8.00013 5.86002V4.66669C10.2068 4.66669 12.0001 6.46002 12.0001 8.66669C12.0001 10.48 10.7801 12.0134 9.1268 12.5C8.8468 12.58 8.6668 12.8467 8.6668 13.1334C8.6668 13.5667 9.08013 13.9067 9.50013 13.78C11.7135 13.1334 13.3335 11.0934 13.3335 8.66669C13.3335 5.72002 10.9468 3.33335 8.00013 3.33335Z"
      fill="#9DA9B8"
    />
    <path
      d="M3.99935 8.66672C3.99935 7.77339 4.29268 6.94672 4.79268 6.27339C4.99268 6.00672 4.96602 5.64006 4.73268 5.40006C4.45268 5.12006 3.97268 5.14672 3.73268 5.46672C3.06602 6.36006 2.66602 7.46672 2.66602 8.66672C2.66602 11.0934 4.28602 13.1334 6.49935 13.7801C6.91935 13.9067 7.33268 13.5667 7.33268 13.1334C7.33268 12.8467 7.15268 12.5801 6.87268 12.5001C5.21935 12.0134 3.99935 10.4801 3.99935 8.66672Z"
      fill="#9DA9B8"
    />
  </svg>
  <svg
    v-if="svgName === 'arrow-right'"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 24 24"
    width="24"
    height="24"
  >
    <path
      d="M1.0625 12.0117H21.5938M21.5938 12.0117L14.535 4.89844M21.5938 12.0117L14.535 19.1005"
      :stroke="fill"
      stroke-width="2"
      stroke-linecap="round"
      stroke-linejoin="round"
    />
  </svg>
</template>

<script setup>
defineProps({
  svgName: {
    type: String,
    default: "",
  },
  width: {
    type: [Number, String],
    default: "20",
  },
  height: {
    type: [Number, String],
    default: "20",
  },
  fill: {
    type: String,
    default: "currentColor",
  },
  className: {
    type: String,
    default: "",
  },
});
</script>
